...
public class ParchisModel {
    ...
    private String spelregel;
    private final List<Kleur> kleuren = Arrays.asList(Kleur.values());
    private Map<Speler, Integer> gegooideWaardes = new HashMap<>();
    private int grootsteWaarde;
    private List<Speler> spelers2 = new ArrayList<>();
    private boolean gooiVoorEersteBeurt;
    private Speler eersteSpeler;
    private boolean normaalSpel;
    private int uitzonderNieuwVakNummer;
    private Pion uitzonderteVerzettenPion;
    ...
    public void start(Map<Integer, String> gekozenKleuren, Map<Integer, String> gekozenNamen) {
        //voeg spelers toe obv keuze in UI
        spelers.voegSpelersToe(gekozenKleuren, gekozenNamen);
        //kies eerste Speler als speler aan zet
        spelerAanZet = spelers.getSpeler(0);
        //je zal nog eerst voor een eerste beurt moeten gooien
        gooiVoorEersteBeurt = true;
        instruction="";
        //geef instructie aan speler aan zet om te gooien.
        getInstructiesGooi(spelerAanZet);
        normaalSpel=true;
    ...
    public String getInstructiesGooi(Speler speler) {
        StringBuilder builder = new StringBuilder();
        if (gooiVoorEersteBeurt) {
            builder.append("Jullie mogen nu één voor één gooien met de dobbelsteen.\nDiegene met de hoogste worp begint\n");
        }
        builder.append(String.format("%s, jij mag gooien%nKlik op de knop \"werp dobbelsteen\"", speler.getNaam()));
        instruction += builder.toString();
        return instruction;
    }

    public Spelers getSpelers() {
        return spelers;
    }
    ...
    public List<Kleur> getKleuren() {
        List<Kleur> kleurenZonderBlanco = new ArrayList<>();
        for (Kleur kleur : kleuren) {
            if (kleur != Kleur.BLANCO) {
                kleurenZonderBlanco.add(kleur);
            }
        }
        return kleurenZonderBlanco;
    }
    ...
    public boolean isGooiVoorEersteBeurt() {
        return gooiVoorEersteBeurt;
    }

    // wie mag beginnen?
    public void speelVoorEersteBeurt(Speler speler) {
        worp = speler.gooi(dobbelsteen);
        instruction = speler.getInstruction();
    ...
    public void kiesEersteSpeler() {
        instruction = String.format("Speler %s heeft het hoogste gegooid en mag dus als eerste beginnen%n", eersteSpeler.getNaam());
        //gooien voor eerste beurt is voorbij.
        gooiVoorEersteBeurt = false;

        //eerste beurt van de speler begint
        eersteSpeler.setAanZet(true);
        //zet aantal worpen van alle spelers terug op 0. Deze worpen tellen niet mee.
        for (Speler speler1 : spelers.getList()) {
            speler1.resetAantalWorpen();
        }
    }
    ...
    public void kiesVolgendeSpeler() {
        ...
        List<Speler> spelersList = spelers.getList();
        //als huidige speler nog recht heeft op een extra beurt, wissel niet
        if (!gooiVoorEersteBeurt) {
            if (!extraBeurt(spelerAanZet.getAantalWorpen())) {
                huidigeSpeler.setAanZet(false);
                //ze aantal worpen van huidige speler terug op nul
            ...
            //indien gooi voor eerste beurt na laatste speler kiesSpeler oproepen
        }
        if(gooiVoorEersteBeurt){
            //ze aantal worpen van huidige speler terug op nul
            huidigeSpeler.resetAantalWorpen();
            if (spelersList.indexOf(huidigeSpeler) != spelersList.size() - 1) {
                ...
                kiesEersteSpeler();
                spelerAanZet=eersteSpeler;
            }
        }
    }

    public boolean isNormaalSpel() {
        return normaalSpel;
    }

    //moeten we in view aan knop hangen en triggered dan methode cheatknop();
    public void setCheaten(boolean cheaten) {
        this.cheaten = cheaten;
    }


    //TO DO: nog toevoegen aan gooi voor eerste beurt.
    public boolean spelersGooienEvenHoog() {
        return Collections.frequency(gegooideWaardes.values(), grootsteWaarde) > 1;
    }

    //TO DO: nog toevoegen aan gooi voor eerste beurt.
    public List<Speler> getSpelers2() {
        spelers2.clear();
        ...
        StringBuilder builder2 = new StringBuilder("het lijkt erop dat zowel ");
        // System.out.print("Het lijkt erop dat zowel ");
        for (int i = 0; i < spelers2.size(); i++) {
            if (i != spelers2.size() - 1) {
                builder2.append(spelers2.get(i).getNaam() + " als ");
            } else {
                builder2.append(spelers2.get(i).getNaam());
            }
        }
        builder2.append(" het hoogste aantal hebben gegooid. Jullie mogen opnieuw gooien.");
        instruction = builder2.toString();
        return spelers2;
    ...
    public void beurt(Speler speler) {
        ...
        if (getAantalZetten(spelerAanZet) != 0) {
            ...
            if (worp == 6) {
                if (spelerAanZet.getAantalWorpen() == 3) {
                    if (laatstVerzettePion.getVak() instanceof LandingsBaan) {
                        normaalSpel=false;
                        instruction += ("Ai je hebt 3 keer 6 gegooid. " +
                                "Je laatst verzette pion moet terug naar begin landingsbaan :(");
                        uitzonderNieuwVakNummer=LandingsBaan.getStartVakNummer(laatstVerzettePion.getKleur());
                        uitzonderteVerzettenPion=laatstVerzettePion;
                    ...
                    if (laatstVerzettePion.getVak() instanceof SpeelVak) {
                        normaalSpel=false;
                        instruction += ("Ai je hebt 3 keer 6 gegooid. Je laatst verzette pion moet terug naar zijn nest :(");
                        uitzonderNieuwVakNummer=spelerAanZet.getBeschikbaarVakNrNest();
                        uitzonderteVerzettenPion=laatstVerzettePion;
                    ...
                    if (spelerAanZet.pionnenUitNest()) {
                        normaalSpel=true;
                        instruction += ("Je hebt 4 pionnen op het speelveld staan en je gooide een 6. " +
                    ...
                    if (spelerAanZet.heeftBarriere()) {
                        ...
                        instruction += String.format("Na het gooien van een 6 ben je verplicht je barriere te verbreken. " +
                                        "1 van deze pionnen is hierdoor verplicht met %d vakjes vooruit gegaan%n"
                                , worp);
                        uitzonderteVerzettenPion = spelers.getPionnenOpVak(barriere).get(0);
                        uitzonderNieuwVakNummer = getNieuwVakNummer(getWorp(), uitzonderteVerzettenPion);
                     //   spelerAanZet.verplaats(teVerzettenPion.getPionNummer(), nieuwVakNr);
                        //als er een pion al op staat , dan mag hij die pion opeten.
                   /*     if (ietsOpTeEten(uitzonderNieuwVakNummer)) {
                            for (Pion pion : spelers.getPionnenOpVak(uitzonderNieuwVakNummer)) {
                                if (pion.getKleur() != spelerAanZet.getKleur()) {
                                    eetPion(pion, uitzonderteVerzettenPion);
                                    break;
                                }
                            }
                        }
            ...
            if (laatstVerzettePion.eindBestemmingBereikt()) {
                normaalSpel=true;
                instruction += String.format("je pion %d heeft eindbestemming bereikt. Je mag indien mogelijk een pion naar keuze" +
                        "verzetten met 10 vakjes%n", laatstVerzettePion.getPionNummer());
                worp = 10;
                toonOpties(spelerAanZet);
            }

        } else instruction += "er zijn geen mogelijke zetten";

        //   } while (extraBeurt(spelerAanZet.getAantalWorpen()) && !spelerAanZet.isWinnaar());
    }

    public void verzetPion(Pion pion) {
        Pion teVerzettenPion = pion;
        int pionNummerTeVerzetten = pion.getPionNummer();
        if(normaalSpel) {
            //check of pion wel verzetbaar is
            if (isVerplaatsbaar(getWorp(), pion)) {
                //verplaats pion:
           ...
           /*     if (ietsOpTeEten(nieuwVakNr)) {
                    for (Pion pion1 : spelers.getPionnenOpVak(nieuwVakNr)) {
                        if (pion1.getKleur() != teVerzettenPion.getKleur()) {
                            eetPion(pion1, teVerzettenPion);
                            break;
                        }
                    }
                }
            ...
            */
                ...
                throw new ParchisException(errorMessage);
            }
        } else{
            spelerAanZet.verplaats(pionNummerTeVerzetten, uitzonderNieuwVakNummer);
        }
    }

    public int getUitzonderNieuwVakNummer() {
        return uitzonderNieuwVakNummer;
    }

    public Pion getUitzonderteVerzettenPion() {
        return uitzonderteVerzettenPion;
    }
    ...
    public boolean extraBeurt(int aantalWorpen) {
        if (worp == 6 && aantalWorpen < 3 && mogelijkeZetten.size() != 0) {
            instruction += "Omdat je 6 gooide, mag je nog een keer";
            return true;
        } else {
            return false;
        }
    ...
    //geeft ofwel true weer ofwel gooit hij een ParchisException
    public boolean isVerplaatsbaar(int worp, Pion pion) {
            ...
            for (Integer safePointVaknr : SpeelVak.getSafePoints()) {
                ...
                if (huidigVakNummer < safePointVaknr && safePointVaknr < nieuwVakNummer) {
                    if (isBarriere(safePointVaknr)) {
                        ...
                        errorMessage=String.format("pion %d kan niet verplaatst worden omdat er een barriere is%n", pion.getPionNummer());
                        return false;
                      //  instruction += String.format("pion %d kan niet verplaatst worden omdat er een barriere is%n", pion.getPionNummer());
                ...
                if (nieuwVakNummer == safePointVaknr) {
                    if (isBarriere(safePointVaknr)) {
                        ...
                        if (pion.getKleur() == getKleurBarriere(safePointVaknr)) {
                            errorMessage=String.format("Er staan reeds 2 pionnen van je kleur op het safepoint");
                            return false;
                        } else {
                            return true;
                        }
        ...
        if (pion.getVak() instanceof LandingsBaan) {
            if (pion.getVakNummer() == LandingsBaan.getEindVakNummer(pion.getKleur())) {
                errorMessage=String.format("pion %d kan niet verplaatst worden omdat deze reeds in centrum staat%n",
                        pion.getPionNummer());
                return false;

            }
            if(pion.getVakNummer() + worp <= LandingsBaan.getEindVakNummer(pion.getKleur())){
                return true;
            } else {
                errorMessage=String.format("pion %d kan enkel uitspelen door het exacte aantal ogen te werpen " +
        ...
        errorMessage=String.format("pion %d zit in nest en je hebt geen 5 gegooit of er staan al 2 pionnen op startvak%n", pion.getPionNummer());
        return false;
    }
    ...
    public void toonOpties(Speler speler) {
        if(normaalSpel) {
            ...
            if (aantalZetten != 0) {
                ...
                StringBuilder builder = new StringBuilder("Dit zijn je mogelijke zetten:");
                for (Pion pion : mogelijkeZetten) {
                    builder.append(String.format("%s kan je verplaatsen naar vak %d%n", pion, getNieuwVakNummer(worp, pion)));
                    instruction = builder.toString();
                }
            } else {
                instruction += "Er zijn geen pionnen die je kan verzetten";
            }
        }else{
    ...
    public int kiesOptie(Speler speler) {
        ...
        if (aantalZetten == 1) {
            ...
            int pionNummer = 0;
            do {
                instruction += "Geef de nummer van je pion die je wil verzetten";
                try {
                    ...
                    if (e instanceof IllegalArgumentException) {
                        ...
                        message = "Er ging iets mis met de keuze welke pion je wil verzetten";
                    }
                    throw new ParchisException(message);
                }
    ...
    public void eetPion(Pion pionReedsOpVak, Pion pionTeVerzetten) {
        ...
        instruction += String.format("ai %s, je pion %d wordt opgegeten door pion %d van speler %s%n",
        ...
        if (isVerplaatsbaar(20, pionTeVerzetten)) {
            instruction += String.format("%s, omdat je pion %d een andere pion heeft opgegeten, mag deze nu" +
    ...
    public void cheatKnop() {
        ...
        spelerAanZet.setAantalBeurten(20);
